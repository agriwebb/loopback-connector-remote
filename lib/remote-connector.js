...
function createProxyMethod(Model, remotes, remoteMethod) {
  ...
  function remoteMethodProxy() {
    ...
    if (
      ...
      lastArg &&
      Object.prototype.hasOwnProperty.call(lastArg, 'awOptions') &&
      typeof lastArg.awOptions === 'object' &&
      lastArg.awOptions
    ) {
      awOptions = args.pop().awOptions
    }

    if (remoteMethod.isStatic) {
      var ctorArgs = [];
      remotes.invoke(remoteMethod.stringName, ctorArgs, args, awOptions, callback);
    } else {
