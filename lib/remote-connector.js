...
function createProxyMethod(Model, remotes, remoteMethod) {
  ...
  function remoteMethodProxy() {
    ...
    var reqOptions;
    if (lastArgIsFunc) {
      if (typeof callback.reqOptions === 'object' && callback.reqOptions) {
        reqOptions = callback.reqOptions;
        delete callback.reqOptions;
      }
    }

    // If reqOptions was not set on the callback, it can be set on the last object-arg of the 
    // called function. In this case we can extract the reqOptions off the object and delete
    // the property
    if (!reqOptions) {
      ...
      if (
        ...
        reqOptions = lastArg.reqOptions
        delete lastArg.reqOptions;

        // Alternatively the reqOptions can be set as a standalone object as the last argument to
        // the function. In this case, we remove the whole object argument from the args that are
        // passed through
